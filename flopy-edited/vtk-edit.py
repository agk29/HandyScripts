...
# Edited by akenny! Use this version
...
class Vtk(object):
    ...
    def write(self, output_file, timeval=None):
        ...
        if (timeval is not None) and (self.vtk_grid_type != "RectilinearGrid"):
...
def export_heads(
    ...
    # get the budget file and the timesteps
    cbb = bf.CellBudgetFile(cbcfile, precision=precision, verbose=verbose)
    totim_dict = dict(zip(cbb.get_kstpkper(), model.dis.get_totim()))
    ...
    for kstpkper_i in kstpkper:
        ...
        vtk.has_cell_data = True # necessary!
        ot_base = ("{}_" + text + "_KPER{}_KSTP{}").format(
            model.name, kstpkper_i[1] + 1, kstpkper_i[0] + 1)
        otfile = os.path.join(otfolder, ot_base)
        vtk.write(otfile, timeval=totim_dict[kstpkper_i])
        # vtk.write(otfile)
        count += 1
...
def export_package(
    ...
    for item, value in pak.__dict__.items():
        ...
        if isinstance(value, list):
            ...
            # if transient list data add to the vtk_trans_dict for later output
            if value.data_type == DataType.transientlist:
                ...
                if value.array is not None:
                    ...
                    # Resize - duplicate for all layers
                    temp = np.empty(vtk.shape)
                    for i in range(vtk.nlay): temp[i] = value.array      
                    vtk.arrays[item] = temp
            ...
            for kper, array_dict in vtk_trans_dict.items():
                ...
                for name, array in sorted(array_dict.items()):
                    if array.array2d:
                        ...
                        temp = np.empty(vtk.shape)
                        for i in range(vtk.nlay): temp[i] = a 
                        vtk.arrays[name] = temp
                    else:
                        a = array.array
                        print("else")
                        print(a.shape)
...
<VTKFile type="Collection" version="0.1"
  ...
  <Collection>\n"""
    ...
    for time in times:
        print(time)
        ucnarr = conc[count,:,:,:]
        vtk.add_array(text, ucnarr)
        vtk.has_cell_data = True # necessary!
        ot_base = ("{}_" + text + "{}").format(
            model.name, count
        )
        otfile = os.path.join(otfolder, ot_base)
        vtk.write(otfile, timeval=time)
